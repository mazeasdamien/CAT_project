...
public class FanucIK : MonoBehaviour
{
    [Header("IK Targets")]
    ...
    [Tooltip("If true, draws debug lines in the Game view using LineRenderers.")]
    ...
    [Tooltip("Width of the runtime debug lines.")]
    public float debugLineWidth = 0.02f;

    [Tooltip("Material for the runtime debug lines. If null, a default sprite material will be used.")]
    ...
    // LineRenderers for runtime visualization
    private LineRenderer[] _targetLines;
    private LineRenderer[] _effectorLines;
    ...
    private void Update()
    {
        ...
        {
            ToggleLines(_targetLines, false);
            ToggleLines(_effectorLines, false);
        }
    }
    ...
    private void UpdateRuntimeLines()
    {
        if (targetTransform != null)
        {
            EnsureLines(ref _targetLines, "TargetAxis");
            UpdateAxisLines(_targetLines, targetTransform.position, targetTransform.rotation);
        }

        if (endEffector != null)
        {
            EnsureLines(ref _effectorLines, "EffectorAxis");
            // Draw End Effector Frame (with offset)
            Quaternion effectiveRot = endEffector.rotation * Quaternion.Euler(rotationOffset);
            UpdateAxisLines(_effectorLines, endEffector.position, effectiveRot);
        }
    }

    private void EnsureLines(ref LineRenderer[] lines, string namePrefix)
    {
        if (lines == null || lines.Length != 3 || lines[0] == null)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                go.transform.SetParent(transform); // Parent to this script's object
    ...
    private void UpdateAxisLines(LineRenderer[] lines, Vector3 pos, Quaternion rot)
    {
        float length = 0.2f;
        ...
        for (int i = 0; i < 3; i++)
        {
            if (lines[i] != null)
            {
                lines[i].startWidth = debugLineWidth;
                lines[i].endWidth = debugLineWidth;
                lines[i].SetPosition(0, pos);
                lines[i].SetPosition(1, pos + (rot * dirs[i] * length));
            }
        }
    }

    private void ToggleLines(LineRenderer[] lines, bool active)
    {
        if (lines != null)
        {
