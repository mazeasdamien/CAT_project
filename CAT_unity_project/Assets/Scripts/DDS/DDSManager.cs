...
public class DDSManager : MonoBehaviour
{
    ...
    private int DomainId => Settings != null ? Settings.DomainId : 0;
    private string ConfigFileName => Settings != null ? Settings.ConfigFileName : "rtps.ini";
    private bool VerboseLogging => Settings?.VerboseLogging ?? true;
    ...
    private void InitializeDDS()
    {
        try
        {
            ...
            Participant = _dpf.CreateParticipant(DomainId);
            if (Participant == null)
            {
    ...
    {
        if (VerboseLogging) Debug.Log($"[DDS-Manager] {msg}");
    }
